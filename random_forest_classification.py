...
# армия Desicion trees , создаем 500 деревьев, потом каждому скармливаем
# данные и во время предсказания мы опрашиваем все 500 деревьев
# и смотри к какая катеогория перевешивает процентно
# чем больше деревьев, тем меньше переобучения
#  microsoft  kinect использовали деревья эти, чтобы определять
#  положение тела человека
# данный алгоритм в нашем случае работает хуже чем, тотже kernel_svm(колокол)
...
# чтобы код выполнился намного быстрее???
